...
Each stage gets its own `Arena`, and the previous stage is discarded as soon as the current stage is available.
...
* Then, we'll check each argument, in order.
  ...
  if so, the argument will have an expected type.
