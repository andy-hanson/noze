...
const Opt<V> tryDeleteAndGet(MutDict<K, V, cmp>* d, const K key) {
	for (const size_t i : Range{mutArrSize(&d->pairs)}) {
		const KeyValuePair<K, V> pair = mutArrAt(&d->pairs, i);
		if (cmp(pair.key, key) == Comparison::equal) {
			deleteAt(d->pairs, i);
			return some<V>(pair.value);
		}
...
template <typename K, typename V, Cmp<K> cmp>
inline const Bool mutDictIsEmpty(const MutDict<K, V, cmp>* d) {
	return mutArrIsEmpty(&d->pairs);
}
